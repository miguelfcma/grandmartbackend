...
export const createProducto = async (req, res) => {
  const { nombre, precio, stock, descripcion,  marca, modelo, color, estado, id_categoria, id_usuario } = req.body;
...
export const updateProducto = async (req, res) => {
  const { nombre, precio, stock, descripcion,  marca, modelo, color, estado, id_categoria, id_usuario } = req.body; // Obtiene los nuevos valores desde el cuerpo de la solicitud
  try {
    //Verifica existencia
    const producto = await Producto.findByPk(req.params.id);
    if (!producto) {
